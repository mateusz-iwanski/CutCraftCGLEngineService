...
namespace CutCraftEngineWebSocketCGLService
{
    internal class Program
    {
        ...
        private static string LinearFlipped(bool aRotated)
        {
            return aRotated ? "; Flipped" : string.Empty;
        }
        ...
        private static void OutputInfo()
        {
            ...
            Console.Write("Hardware Code: {0}\n", calculator.ComputerHardwareCode());
        }
        ...
        private static void OutputLinearResults_by_Parts(CutEngine aCalculator)
        {
            ...
            bool rotated, flipped, isAngle = aCalculator.CrossSection > 0;
            string id;
            ...
            for (int iPart = 0; iPart < aCalculator.PartCount; iPart++)
            {
                ...
                if (isAngle)
                {
                    if (aCalculator.GetResultLinearPart(iPart, out StockNo, out Length, out angleStart, out angleEnd, out X, out rotated, out flipped, out id))
                    {
                        Console.Write("Part ID={0};  stock={1};  X={2};  Length={3}; AngleStart={4}; AngleEnd={5}{6}{7}\n",
                                      id, StockNo, X, Length, angleStart, angleEnd, LinearTurn(rotated), LinearFlipped(flipped));
                    }
        ...
        private static void OutputLinearResults_by_Layout(CutEngine aCalculator)
        {
            ...
            bool rotated, flipped, StockActive;
            ...
            for (iLayout = 0; iLayout < aCalculator.LayoutCount; iLayout++)
            {
                ...
                if (StockCount > 0)
                {
                    ...
                    // aCalculator.CreateLinearStockImage(StockIndex, string.Format("d:\\Linear_Layout_{0}.jpg", iLayout + 1), 2000);
                    ...
                    for (iStock = StockIndex; iStock < StockIndex + StockCount; iStock++)
                    {
                        if (aCalculator.GetLinearStockInfo(iStock, out StockLength, out StockActive))
                        {
                            ...
                            for (iPart = 0; iPart < partCount; iPart++)
                            {
                                ...
                                if (isAngle)
                                {
                                    aCalculator.GetResultLinearPart(partIndex, out tmp, out partLength, out angleStart, out angleEnd, out X, out rotated, out flipped);
                                    // Output the part information
                                    Console.Write("Part= {0}:  X={1};  Length={2}; AngleStart={3}; AngleEnd={4}{5}{6}\n",
                                                  partIndex, X, partLength, angleStart, angleEnd, LinearTurn(rotated), LinearFlipped(flipped));
        ...
        */
        private static void LinearMultipleSheetSize()
        {
            ...
            Calculator.LinearMaxSizePerStock = 2; // Allow only 2 differen sizes per stock
            Calculator.LinearMaxSizePerStockThreshold = 0.6; // If the ration less then 0.6 (60%) then allow more sizes
        ...
        private static void LinearAngleCuts()
        {
            ...
            Calculator.LinearAllowFlipping = true;
            // Run the calculation:
            string result = Calculator.ExecuteLinear();
            if (result == "")
            {
        ...
        private static void OutputSheetResults_by_Layout(CutEngine aCalculator)
        {
            ...
            for (iLayout = 0; iLayout < aCalculator.LayoutCount; iLayout++)
            {
                ...
                {
                    // Uncomment and change the file name if required
                    // aCalculator.CreateStockImage(sheetIndex, string.Format("d:\\Panel_Layout_{0}.jpg", iLayout + 1), 2000);
        ...
        */
        private static void SheetOneSheetSize()
        {
            ...
            Calculator.AddStock(2400, 2000, 5);
            // Add parts we have to cut from the sheet:
            Calculator.AddPart(640, 420, 9, true, "640x420mm"); // 9 parts of 640x420 mm
            Calculator.AddPart(150, 720, 14, false, "150x720mm"); // 14 non-rotatable parts of 150x720 mm
            Calculator.AddPart(1000, 420, 12, true, "1000x420mm"); // 12 parts of 1000x420 mm
                                                                   // Run the calculation:
        ...
        */
        ...
        public static void OnCalcState(ref bool aStop)
        {
            CalcCount++;
            // Stop calculation after 10 calls.
            if (CalcCount > 10)
            {
                aStop = true;
            }
        ...
        /*
          This example demonstrates how to stop a calculaiton by using a callback function.
        */
        private static void StopCalculation()
        {
            Console.Write("\n===============================================================\n");
            // First we create a new instance of the cut engine
            CutEngine Calculator = new CutEngine();
            Calculator.AddStock(2400, 2000, 100);
            Calculator.AddStock(1700, 1400, 100);
            Calculator.AddStock(2100, 1700, 100);
            Calculator.AddStock(1000, 2300, 100);
            Calculator.AddStock(2100, 1000, 100);
            // Add parts we have to cut from the sheet:
            Calculator.AddPart(640, 420, 900);
            Calculator.AddPart(150, 720, 730);
            Calculator.AddPart(1000, 420, 250);
            Calculator.AddPart(350, 722, 930);
            Calculator.AddPart(150, 560, 990);
            Calculator.AddPart(551, 725, 1000);
            // Run the calculation:
            CalcCount = 0;
            string result = Calculator.Execute(OnCalcState);
        }
